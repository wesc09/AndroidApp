...
public class DBOpenHelper extends OpenMRSSQLiteOpenHelper {
    ...
    public int updatePatient(SQLiteDatabase db, long patientID, Patient patient) {
        ...
        if (null != patient.getPerson().getVariable()){
            newValues.put(PatientTable.Column.WATER, patient.getPerson().getVariable().getWater());
            newValues.put(PatientTable.Column.AREA, patient.getPerson().getVariable().getArea());
            newValues.put(PatientTable.Column.LIGHT, patient.getPerson().getVariable().getLight());
            newValues.put(PatientTable.Column.ENERGY, patient.getPerson().getVariable().getEnergy());
            newValues.put(PatientTable.Column.PEOPLE, patient.getPerson().getVariable().getPeople());
            newValues.put(PatientTable.Column.SONS, patient.getPerson().getVariable().getSons());
            newValues.put(PatientTable.Column.GPS, patient.getPerson().getVariable().getGps());
            newValues.put(PatientTable.Column.LENGUAGE, patient.getPerson().getVariable().getLenguage());
            newValues.put(PatientTable.Column.ILLITERATE, patient.getPerson().getVariable().getIlliterate());
            newValues.put(PatientTable.Column.JOB, patient.getPerson().getVariable().getJob());
        }
