...
public class PatientDAO {
    ...
    private Patient cursorToPatient(Cursor cursor) {
        ...
        person.getVariables().add(cursorToVariables(cursor));
    ...
    private PersonVariables cursorToVariables(Cursor cursor) {

        int waterColumnIndex = cursor.getColumnIndex(PatientTable.Column.WATER);
        int areaColumnIndex = cursor.getColumnIndex(PatientTable.Column.AREA);
        int lightColumnIndex = cursor.getColumnIndex(PatientTable.Column.LIGHT);
        int energyColumnIndex = cursor.getColumnIndex(PatientTable.Column.ENERGY);
        int peopleColumnIndex = cursor.getColumnIndex(PatientTable.Column.PEOPLE);
        int sonsColumnIndex = cursor.getColumnIndex(PatientTable.Column.SONS);
        int gpsColumnIndex = cursor.getColumnIndex(PatientTable.Column.GPS);
        int lenguageColumnIndex = cursor.getColumnIndex(PatientTable.Column.LENGUAGE);
        int illiterateColumnIndex = cursor.getColumnIndex(PatientTable.Column.ILLITERATE);
        int jobColumnIndex = cursor.getColumnIndex(PatientTable.Column.JOB);

        PersonVariables personVariables = new PersonVariables();

        personVariables.setWater(cursor.getString(waterColumnIndex));
        personVariables.setArea(cursor.getString(areaColumnIndex));
        personVariables.setLight(cursor.getString(lightColumnIndex));
        personVariables.setEnergy(cursor.getString(energyColumnIndex));
        personVariables.setPeople(cursor.getString(peopleColumnIndex));
        personVariables.setSons(cursor.getString(sonsColumnIndex));
        personVariables.setGps(cursor.getString(gpsColumnIndex));
        personVariables.setLenguage(cursor.getString(lenguageColumnIndex));
        personVariables.setIlliterate(cursor.getString(illiterateColumnIndex));
        personVariables.setJob(cursor.getString(jobColumnIndex));
